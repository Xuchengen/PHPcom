...
FileProgress.prototype.reset = function () {
	...
	this.fileProgressElement.childNodes[0].style.width = "0%";
...
FileProgress.prototype.setComplete = function () {
	...
	this.fileProgressElement.childNodes[0].style.width = "100%";
...
function fileQueued(file) {
	try {
		...
		progress.setStatus('<font color="red">' + size + '</font> 等待上传......');
...
function fileQueueError(file, errorCode, message) {
	try {
		if (errorCode === SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED) {
			alert("您的上传列队文件过多.\n" + (message === 0 ? "已经达到上传限制." : "您可以选择 " + (message > 1 ? " " + message + " 个文件." : "一个文件.")));
			return;
		}
...
function uploadStart(file) {
	try {
		...
		var progress = new FileProgress(file, this.customSettings.progressTarget);
		var size = formatBytes(file.size);
		progress.setStatus('<font color="red">' + size + '</font> 上传开始......');
		progress.toggleCancel(true, this);
	}
	catch (ex) {}
...
function uploadProgress(file, bytesLoaded, bytesTotal) {
	try {
		...
		progress.setStatus('<font color="red">' + size + '</font> 正在上传......' + percent + '%');
	} catch (ex) {
		this.debug(ex);
	}
}
...
function uploadSuccess(file, serverData) {
	try {
		...
		progress.setStatus('<font color="red">' + size + '</font> 上传成功.');
		progress.toggleCancel(false);
	} catch (ex) {
		this.debug(ex);
	}
...
function uploadError(file, errorCode, message) {
	try {
		...
		case SWFUpload.UPLOAD_ERROR.FILE_VALIDATION_FAILED:
			progress.setStatus("未通过有效性验证。上传跳过。");
