...
class Soft_Type extends Controller_ThreadList
{
	public function loadActionIndex()
	{
		...
		/*$sql = "SELECT * FROM " . DB::table('threads') . " WHERE rootid='29' LIMIT 200";
		$query = DB::query($sql);
		$datas = array();
		while ($row = DB::fetch_array($query)) {
			$datas[$row['tid']]['tid'] = $row['tid'];
			$datas[$row['tid']]['classid'] = 33;
			$datas[$row['tid']]['catid'] = $row['catid'];
			$datas[$row['tid']]['dateline'] = $row['dateline'];
		}
		foreach ($datas as $cid => $data){
			//DB::insert("thread_class_data", $data);
		}*/
		//DB::update('thread_class_data', array('classid' => 33), "classid='40'");
		return 1;
	}
}
?>