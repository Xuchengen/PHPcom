...
class Video_Index extends Controller_MainAbstract
{
	public function loadActionIndex()
	{
		...
		if(isset(phpcom::$G['channel']['module'][$modname])){
			...
			if(phpcom::$G['channel'][$chanid]['modules'] !== 'video'){
				$chanid = 5;
			}
		}
		$this->chanid = $chanid;
		...
		$tplname = 'video/index_' . phpcom::$G['cache']['channel']['codename'];
		if(!tplfile_exists($tplname)){
			$tplname = checktplname('video/index', $this->chanid);
		}
