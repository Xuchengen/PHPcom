 ...
 * This File   : member.php    2011-8-26 11:57:20
 */
!defined('IN_PHPCOM') && exit('Access denied');
define('USER_CHECK_INVITECODE_INVALID', 0);
define('USER_CHECK_INVITECODE_EXPIRES', -1);

function check_allowsregip($ip = '') {
    $allowsregip = trim(phpcom::$setting['allowsregip']);
    if (empty($allowsregip)) {
        return -1;
    }
...
function check_username($username, $minlen = 3, $maxlen = 20) {
    $regexp = '\xA1\xA1|\xAC\xA3|^Guest|^\xD3\xCE\xBF\xCD|\xB9\x43\xAB\xC8';
    $len = getstrlen($username);
...
function update_invite($username, $invite = array()) {
    if ($username && $invite && is_array($invite)) {
        ...
        if ($invite['type'] == 1 && $reward && $uid) {
            DB::query("UPDATE " . DB::table('members') . " SET $field=$field+'$reward' WHERE uid='$uid'", 'UNBUFFERED');
        }
        $reward = intval(phpcom::$setting['invite']['inviteecredit']);
        if ($reward) {
            DB::query("UPDATE " . DB::table('members') . " SET $field=$field+'$reward' WHERE username='$username'", 'UNBUFFERED');
        }
        if ($groupid > 7) {
...
function userlogin($username, $password, $questionid = '', $answer = '', $loginmode = 'username') {
    $result = array();
    ...
    $result['ucenter'] = phpcom::addslashes($result['ucenter']);
...
function setmemberlogin($member, $cookietime = 0) {
...
function replace_sitevar($string, $replaces = array()) {
    $sitevars = array(
...
function logincheck($username) {
    ...
    $return = (!$login || (TIMESTAMP - $login['lastupdate'] > 900)) ? 4 : max(0, 4 - $login['logincount']);
    ...
    if (!$login) {
        ...
        DB::query("REPLACE INTO " . DB::table('failedlogin') . " (ip, username, logincount, lastupdate) VALUES ('$clientip', '$username', '0', '$timestamp')");
        DB::query("DELETE FROM " . DB::table('failedlogin') . " WHERE lastupdate<$timestamp-901", 'UNBUFFERED');
    }
    return $return;
}
...
function member_add($username, $password, $email, $uid = 0, $groupid = 11, $questionid = '', $answer = '', $regip = '', $salt = '') {
...
function member_edit($uid, $members = array(), $membercounts = array(), $memberinfos = array(), $memberstatus = array(), $password = '', $chkpasswd = FALSE) {
