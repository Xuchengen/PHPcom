...
if (!checksubmit(array('btnsubmit', 'submit_button'), 1)) {
    $adminhtml->form("m=article&action=list&chanid=$chanid&page=$page", null, 'onkeydown="return formdown()"');
    $adminhtml->table_header();
    $adminhtml->table_th(array(
        array('', 'width="2%" align="center" noWrap="noWrap"'),
        array('article_title', 'width="53%" class="left" noWrap="noWrap"'),
    ...
    $sql = DB::buildlimit("SELECT t.*,c.basic,c.catname,c.codename,c.prefixurl,c.prefix,c.color,t2.editor 
    		...
    		LEFT JOIN " . DB::table('article_thread') . " t2 USING(tid) 
			WHERE $condition ORDER BY t.dateline DESC", $pagesize, $pagestart);
    $query = DB::query($sql);
    while ($row = DB::fetch_array($query)) {
        $tid = $row['tid'];
        $edit = $adminhtml->edit_word('edit', "m=article&action=edit&chanid=$chanid&tid=$tid", ' | ');
        $edit .= $adminhtml->edit_word('attachment', "m=attachment&chanid=$chanid&tid=$tid");
        //$edit .= $adminhtml->del_word('delete', "m=article&action=del&chanid=$chanid&tid=$tid");
        ...
        $row['viewurl'] = "?m=article&action=view&chanid=$chanid&tid=$tid";
        if (empty($row['url'])) {
        ...
        $adminhtml->table_td(array(
            ...
            array('<a href="' . $row['url'] .'" target="_blank"><img src="misc/images/icons/' . $row['icons'] . '" /></a> <a class="lst" href="?m=article&action=list&chanid=' . $chanid . '&catid=' . $row['catid'] . '"' . $row['color'] . '>' . $row['catname'] . '</a>
					&#8226; <a class="lst" href="' . $row['viewurl'] .'"'. $row['highlight'] . '>' . $row['title'] . '</a> ' . $row['digest'] . $row['focus'] . $row['topline'] . $row['auditicon'] . $row['locked'], TRUE),
    ...
    $showpage = '<var class="morePage">' . showpage($pagenow, $pagecount, $pagesize, $totalrec, ADMIN_SCRIPT . "?m=article&action=list&chanid=$chanid$queryurl") . '</var>';
    $adminhtml->table_td(array(
	...
	$returnurl = "m=article&action=list&chanid=$chanid&page=$page";
	...
	if(strcasecmp($operation, 'digest') === 0){
		...
		if(empty($deleted)){
			...
			if (delete_article_thread($checkboxes)) {
		...
		if(empty($moved)){
			...
			$sql = "SELECT channelid, channelname FROM " . DB::table('channel') . " WHERE modules='article'";
			$query = DB::query($sql);
			while ($row = DB::fetch_array($query)) {
			...
			if($rootid = get_rootid($catid, $channelid)){
				$num = $none = 0;
				foreach ($checkboxes as $tid){
					if(threadmove($tid, $catid, $rootid, $channelid, 'article')){
						$num++;
					}else{
						$none++;
					}
