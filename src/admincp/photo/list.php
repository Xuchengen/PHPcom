...
if (!checksubmit(array('btnsubmit', 'submit_button'), 1)) {
	$adminhtml->form("m=photo&action=list&chanid=$chanid&page=$page", null, 'onkeydown="return formdown()"');
	$adminhtml->table_header();
	$adminhtml->table_th(array(
			array('', 'width="2%" align="center" noWrap="noWrap"'),
			array('photo_title', 'width="53%" class="left" noWrap="noWrap"'),
	...
	$sql = DB::buildlimit("SELECT t.*,c.basic,c.catname,c.codename,c.prefixurl,c.prefix,c.color,t2.editor 
			FROM " . DB::table('threads') . " t
			LEFT JOIN " . DB::table('category') . " c USING(catid) 
			LEFT JOIN " . DB::table('photo_thread') . " t2 USING(tid) 
			WHERE $condition ORDER BY t.dateline DESC", $pagesize, $pagestart);
	$query = DB::query($sql);
	while ($row = DB::fetch_array($query)) {
		$tid = $row['tid'];
		$edit = $adminhtml->edit_word('edit', "m=photo&action=edit&chanid=$chanid&tid=$tid", ' | ');
		$edit .= $adminhtml->edit_word('attachment', "m=attachment&chanid=$chanid&tid=$tid");
		//$edit .= $adminhtml->del_word('delete', "m=photo&action=del&chanid=$chanid&tid=$tid");
		...
		$row['auditicon'] = $row['status'] == 1 ? '' : '<a href="?m=photo&action=audit&tid='.$tid.'&chanid='.$chanid.'"><img src="misc/images/icons/audit.gif" /></a>';
		switch ($row['digest']) {
		...
		$adminhtml->table_td(array(
				...
				array('<img src="misc/images/icons/' . $row['icons'] . '" /> <a class="lst" href="?m=photo&action=list&chanid=' . $chanid . '&catid=' . $row['catid'] . '"' . $row['color'] . '>' . $row['catname'] . '</a>
					&#8226; <a class="lst" href="' . $row['url'] .'" target="_blank"'. $row['highlight'] . '>' . $row['title'] . '</a> ' . $row['digest'] . $row['focus'] . $row['topline'] . $row['auditicon'] . $row['locked'], TRUE),
	...
	$showpage = '<var class="morePage">' . showpage($pagenow, $pagecount, $pagesize, $totalrec, ADMIN_SCRIPT . "?m=photo&action=list&chanid=$chanid$queryurl") . '</var>';
	$adminhtml->table_td(array(
	...
	$returnurl = "m=photo&action=list&chanid=$chanid&page=$page";
	...
	if(strcasecmp($operation, 'digest') === 0){
		...
		if(empty($deleted)){
			...
			if (delete_photo_thread($checkboxes)) {
		...
		if(empty($moved)){
			...
			$sql = "SELECT channelid, channelname FROM " . DB::table('channel') . " WHERE modules='photo'";
			$query = DB::query($sql);
			while ($row = DB::fetch_array($query)) {
			...
			if($rootid = get_rootid($catid, $channelid)){
				$num = $none = 0;
				foreach ($checkboxes as $tid){
					if(threadmove($tid, $catid, $rootid, $channelid, 'photo')){
						$num++;
					}else{
						$none++;
					}
